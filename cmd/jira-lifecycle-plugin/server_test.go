	...
	for _, testCase := range testCases {
		t.Run(testCase.title, func(t *testing.T) {
			...
			if notFound != testCase.expectedNotFound {
				t.Errorf("%s: unexpected notFound %t != %t", testCase.title, notFound, testCase.expectedNotFound)
			}
			if noJira != testCase.expectedNoJira {
				t.Errorf("%s: unexpected noJira %t != %t", testCase.title, noJira, testCase.expectedNoJira)
			}
			if upstreamSync != testCase.expectedUpstreamSync {
				t.Errorf("%s: unexpected upstreamSync %t != %t", testCase.title, upstreamSync, testCase.expectedUpstreamSync)
			}
		})
	}
}
